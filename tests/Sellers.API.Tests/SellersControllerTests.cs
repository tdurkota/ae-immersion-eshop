...
using Sellers.API.Infrastructure;
using Sellers.API.Responses;
...
public class SellersControllerTests
{
    ...
    public SellersControllerTests()
    {
        ...
        _controller = new SellersController(_context, _logger, _payoutRepository);
    }
    ...
    public async Task GetSellerOrders_UnauthorizedSeller_ReturnsForbidden()
    {
        ...
        // Mock different seller authentication
    ...
    [TestMethod]
    [Description("Task 10.1: GET /api/sellers/{id}/orders - Data isolation: seller only sees their orders")]
    public async Task GetSellerOrders_DataIsolation_SellerSeesOnlyOwnOrders()
    {
        ...
        // Seller2 has items in Order 2 (different order)
        var payout2 = new SellerPayout
        {
            PayoutId = Guid.NewGuid(),
            SellerId = sellerId2,
        ...
        var claims = new[] { new Claim("seller_id", sellerId1.ToString()) };
        var identity = new ClaimsIdentity(claims, "TestAuth");
        var principal = new ClaimsPrincipal(identity);
        _controller.ControllerContext.HttpContext = new DefaultHttpContext { User = principal };
        ...
        // Seller1 should see only their order (Order 1), not seller2's orders
        Assert.AreEqual(1, root.GetProperty("total").GetInt32());
    }
    ...
    [TestMethod]
    [Description("Task 10.2: GET /api/sellers/{id}/payouts - Filter by status")]
    ...
    [TestMethod]
    [Description("Task 10.2: GET /api/sellers/{id}/payouts - Filter by date range")]
    public async Task GetSellerPayouts_FilterByDateRange_ReturnsDateFilteredPayouts()
    {
        // Arrange
        var sellerId = Guid.NewGuid();
        var seller = new Seller
        ...
        // Act
        var result = await _controller.GetSellerPayouts(sellerId, null, today, today, 1, 20, CancellationToken.None);
    ...
    public async Task GetSellerPayoutSummary_WithPayouts_ReturnsCorrectTotals()
    {
        // Arrange
        var sellerId = Guid.NewGuid();
        var seller = new Seller
        ...
        Assert.AreEqual(255, root.GetProperty("TotalEarned").GetDecimal());
        Assert.AreEqual(85, root.GetProperty("TotalPending").GetDecimal());
        Assert.AreEqual(0, root.GetProperty("TotalProcessed").GetDecimal());
        Assert.AreEqual(170, root.GetProperty("TotalPaid").GetDecimal());
    }
